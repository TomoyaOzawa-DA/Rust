...
function_inner_loop <- function(grid, params_theta_1, method = "linear", beta, params_theta_3,threshold = 0.01){
  ...
  EV <- matrix(0, grid, iteration_max) # 初期値
  ...
  for (i in 1: (iteration_max - 1)){
    ...
    if (min(abs(EV[, i+1] - EV[, i])) < threshold){ # 差分が閾値より下回れば不動点に達したとみなす。収束条件を検討する必要あり
      position <- i+1
  ...
  if(!achieved){
    cat("Convergence NOT achieved in ",min(abs(EV[, iteration_max] - EV[, iteration_max-1]))) # 繰り返し回数の最大値を超えても収束しなかった場合に警告する
    position <- iteration_max
  }
